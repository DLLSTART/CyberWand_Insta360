"""
CyberWand v20.0 - 体积最小化版
基于 v19.0，长度减少 30%，最小化设计

关键变更：
1. 总长度：500mm → 350mm（-30%）
2. 手柄长度：15mm → 12mm
3. 手柄直径：28mm → 22mm（刚好容纳 PCB）
4. 杖身直径：21-12mm → 14-8mm（最小化）
5. 保持 6 个瘤体
"""
...
# 魔杖参数（最小化，-30%）
wand_total_length = 350  # 500mm × 0.7 = 350mm
handle_length = 12  # 手柄长 12mm（最小化）
handle_diameter = 22  # 手柄直径 22mm（刚好容纳 PCB 19×18mm）
shaft_base_diameter = 14  # 杖身底部直径 14mm（最小化）
shaft_tip_diameter = 8  # 杖身顶端直径 8mm

# 瘤体参数（6 个，适配缩短的杖身）
num_nodes = 6
max_bump_dia = 18.0  # 尾部最大瘤体直径
min_bump_dia = max_bump_dia * (1 - 3/7)  # 10.3mm 顶端最小
min_spacing = 28  # 底部最短间隔（适配缩短的杖身）
max_spacing = 42  # 顶端最长间隔

# 生成渐进变化的瘤体数据（6 个，适配 350mm 总长）
node_data = []
...
def draw_annotations():
    ...
    annotations = [
        ("总长度：350mm（-30% 最小化）", center_x - 250, center_y + 420),
        ("手柄：Φ22mm × 12mm（最小化，容纳 PCB）", center_x - 250, center_y + 450),
        ("杖身：Φ14-8mm（最小化）", center_x - 250, center_y + 480),
        ("瘤体数量：6 个（间隔 28-42mm）", center_x - 250, center_y + 510),
        ("尾部瘤体：18mm + 间隔 28mm", center_x - 250, center_y + 540),
        ("顶端瘤体：10.3mm + 间隔 42mm", center_x - 250, center_y + 570),
    ]
...
def draw_title():
    ...
    title = "CyberWand v20.0 - 体积最小化版"
    subtitle = "总长 350mm（-30%）| 手柄Φ22mm | 6 个瘤体"
...
# 执行绘制
print("Drawing compact minimal wand v20.0...")
draw_wand()
draw_annotations()
draw_title()

# 保存
output_path = r"D:\workspace\CyberWand_Insta360\enclosure\wand_v200_compact_minimal.png"
