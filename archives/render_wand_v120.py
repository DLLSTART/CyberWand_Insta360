"""
CyberWand v12.0 - 尾部加粗加密版
修正：
1. 尾部瘤体直径扩大 1/5: 12mm → 14.4mm
2. 尾部瘤体间隔靠近：35mm → 28mm
"""
...
# 瘤体参数（尾部加粗加密）
num_nodes = 9
max_bump_dia = 12.0 * 1.2  # 14.4mm（扩大 1/5）
min_bump_dia = max_bump_dia * (1 - 3/7)  # 8.23mm 顶端最小
min_spacing = 28  # 底部最短间隔（靠近到 28mm）
...
for i in range(num_nodes):
    ...
    # 间隔渐进：底部短 → 顶端长（尾部 3 个之间保持 28mm 间隔）
    if i < 3:
        spacing = min_spacing  # 尾部 3 个之间保持 28mm
    else:
...
def draw_annotations():
    ...
    annotations = [
        ...
        ("尾部瘤体：14.4mm + 间隔 28mm（扩大 1/5，加密）", center_x - 250, center_y + 520),
        ("顶端瘤体：8.2mm + 间隔 55mm", center_x - 250, center_y + 555),
...
def draw_title():
    ...
    title = "CyberWand v12.0 - 尾部加粗加密版"
    subtitle = "尾部直径 +1/5(14.4mm) | 间隔 -28mm"
...
# 执行绘制
print("Drawing thicker denser tail wand v12.0...")
draw_wand()
draw_annotations()
draw_title()

# 保存
output_path = r"D:\workspace\CyberWand_Insta360\enclosure\wand_v120_thicker_tail.png"
