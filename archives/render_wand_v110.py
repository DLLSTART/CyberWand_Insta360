"""
CyberWand v11.0 - 圆柱手柄版
修正：
1. 手柄尾部是圆柱形，不是圆球
2. 手柄到第一个瘤体间距合理（30mm 起始）
"""
...
# 瘤体参数
num_nodes = 9  # 总共 9 个瘤体（尾部 3 个 + 中间 6 个）
max_bump_dia = 12.0  # 底部最大
min_bump_dia = max_bump_dia * (1 - 3/7)  # 6.86mm 顶端最小
min_spacing = 35  # 底部最短间隔（尾部 3 个之间保持 35mm）
...
for i in range(num_nodes):
    ...
    # 间隔渐进：底部短 → 顶端长（尾部 3 个之间保持 35mm 间隔）
    if i < 3:
        spacing = min_spacing  # 尾部 3 个之间保持 35mm
    else:
...
def draw_wand():
    ...
    for i, node in enumerate(node_data):
        # 从手柄末端开始计算位置
        t = node["pos"] / (wand_total_length - 80)
...
def draw_annotations():
    ...
    annotations = [
        ...
        ("手柄：圆柱形 16mm（不是圆球）", center_x - 250, center_y + 485),
        ("底部瘤体：12mm + 间隔 30mm（圆柱形）", center_x - 250, center_y + 520),
        ("顶端瘤体：6.9mm + 间隔 55mm（椭球形）", center_x - 250, center_y + 555),
...
def draw_title():
    ...
    title = "CyberWand v11.0 - 圆柱手柄版"
    subtitle = "手柄圆柱形 + 间距合理 30mm 起始"
...
# 执行绘制
print("Drawing cylindrical handle wand v11.0...")
draw_wand()
draw_annotations()
draw_title()

# 保存
output_path = r"D:\workspace\CyberWand_Insta360\enclosure\wand_v110_cylindrical_handle.png"
