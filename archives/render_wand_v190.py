"""
CyberWand v19.0 - v15.0 减瘤体版
基于 v15.0 等比例放大版，减少 30% 瘤体
...
handle_diameter = 16 * SCALE_FACTOR  # 28mm
shaft_base_diameter = 12 * SCALE_FACTOR  # 21mm
shaft_tip_diameter = 7 * SCALE_FACTOR  # 12.25mm

# 瘤体参数（v15.0 9 个 → v19.0 6 个，减少 30%）
num_nodes = 6  # 减少到 6 个
max_bump_dia = (12.0 * 0.8) * SCALE_FACTOR  # 16.8mm
min_bump_dia = max_bump_dia * (1 - 3/7)  # 9.6mm
min_spacing = 38  # 底部最短间隔
max_spacing = 55  # 顶端最长间隔

# 生成渐进变化的瘤体数据（6 个，间距加大）
node_data = []
for i in range(num_nodes):
    ...
    # 位置计算（6 个瘤体，总跨度约 280mm）
    if i == 0:
        pos = 60  # 第一个瘤体距手柄末端 60mm
    else:
...
def draw_annotations():
    ...
    annotations = [
        ...
        ("手柄直径：28mm（v13.0 16mm × 1.75）", center_x - 280, center_y + 485),
        ("杖身直径：21-12.25mm（v13.0 12-7mm × 1.75）", center_x - 280, center_y + 520),
        ("瘤体数量：6 个（v15.0 9 个减少 30%）", center_x - 280, center_y + 555),
        ("尾部瘤体：16.8mm + 间隔 38mm（2 个圆柱形）", center_x - 280, center_y + 590),
        ("顶端瘤体：9.6mm + 间隔 55mm", center_x - 280, center_y + 625),
    ]
...
def draw_title():
    ...
    title = "CyberWand v19.0 - v15.0 减瘤体版"
    subtitle = "6 个瘤体（-30%）| 放大 1.75× | 手柄 28mm"
...
# 执行绘制
print("Drawing v15.0 with 30% fewer nodes v19.0...")
draw_wand()
draw_annotations()
draw_title()

# 保存
output_path = r"D:\workspace\CyberWand_Insta360\enclosure\wand_v190_fewer_nodes.png"
