...
# 圆球节点参数（修正比例：圆球直径 = 杖身直径 × 1.3-1.6 倍）
node_data = [
    {"pos": 25, "shaft_dia": 5.5, "node_dia": 7.5, "spacing": 8},   # 节点 1: 1.36 倍
    {"pos": 18, "shaft_dia": 4.5, "node_dia": 6.8, "spacing": 7},   # 节点 2: 1.51 倍
    {"pos": 12, "shaft_dia": 4.0, "node_dia": 6.0, "spacing": 6},   # 节点 3: 1.50 倍
    {"pos": 7, "shaft_dia": 3.5, "node_dia": 5.2, "spacing": 5},    # 节点 4: 1.49 倍
    {"pos": 3, "shaft_dia": 3.0, "node_dia": 4.5, "spacing": 4},    # 节点 5: 1.50 倍
    {"pos": 0, "shaft_dia": 2.5, "node_dia": 3.8, "spacing": 3},    # 节点 6: 1.52 倍
]
...
def draw_irregular_node(cx, cy, shaft_radius, node_radius):
    ...
    for i in range(num_points):
        ...
        radius_variation = node_radius * (1 + 0.15 * math.sin(angle_i * 3) + 0.1 * math.cos(angle_i * 5))
        x = cx + math.cos(angle_i) * radius_variation * 1.2
        y = cy + math.sin(angle_i) * radius_variation * 0.9
...
def draw_wand():
    ...
    for i, node in enumerate(node_data):
        ...
        draw_irregular_node(nx, ny, shaft_radius, node_radius)
...
def draw_title():
    ...
    title = "CyberWand v3.7 - 圆球比例修正版"
    subtitle = "圆球直径 = 杖身直径 × 1.3-1.6 倍（协调美观）"
...
# 执行绘制
print("Drawing wand with corrected proportions...")
draw_wand()
draw_title()

# 保存
output_path = r"D:\workspace\CyberWand_Insta360\enclosure\wand_v37_correct_proportions.png"
