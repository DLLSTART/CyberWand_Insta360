"""
CyberWand v14.0 - PCB 兼容版
关键修正：容纳 PCB、LED、电池、按键、USB-C

尺寸设计依据：
- PCB 尺寸：25mm 宽 × 35mm 长 × 1.6mm 厚
- 电池：300mAh 锂电池 (30×20×5mm)
- LED：WS2812B (5×5mm) 在杖尖
- 按键：6mm 轻触开关
- USB-C：16Pin Type-C 接口
- 壁厚：单边 1.5mm，总壁厚 3mm

外壳最小内径计算：
- 手柄内径：PCB 宽度 25mm + 壁厚 3mm = 28mm
- 杖身内径：电池宽度 20mm + 壁厚 3mm = 23mm
- 外部直径：内径 + 壁厚 = 手柄 31mm / 杖身 26mm
"""

import numpy as np
from PIL import Image, ImageDraw, ImageFont
import math
import random

# 创建画布
width, height = 3840, 2160
img = Image.new('RGBA', (width, height), (245, 245, 240, 255))
draw = ImageDraw.Draw(img)

# 中心参数
center_x, center_y = width // 2, height // 2
angle = -35
rad = math.radians(angle)
scale = 6

# 魔杖参数（PCB 兼容尺寸）
wand_total_length = 500
handle_diameter_external = 31  # 手柄外径 31mm（内径 28mm 容纳 PCB）
handle_diameter_internal = 28  # 手柄内径 28mm
shaft_base_diameter_external = 26  # 杖身底部外径 26mm（内径 23mm 容纳电池）
shaft_base_diameter_internal = 23
shaft_tip_diameter_external = 15  # 杖尖外径 15mm（LED 位置）
shaft_tip_diameter_internal = 12

# 瘤体参数（适配加粗的杖身）
num_nodes = 9
max_bump_dia = 32.0  # 尾部最大瘤体直径（比杖身外径大 6mm）
min_bump_dia = max_bump_dia * (1 - 3/7)  # 18.3mm 顶端最小
min_spacing = 38  # 底部最短间隔
max_spacing = 55  # 顶端最长间隔

# 生成渐进变化的瘤体数据
node_data = []
for i in range(num_nodes):
    t = i / (num_nodes - 1)
    progress = t * t
    
    # 直径渐进：底部大 → 顶端小
    bump_dia = max_bump_dia * (1 - progress) + min_bump_dia * progress
    
    # 间隔渐进：底部短 → 顶端长（尾部 3 个之间保持 38mm 间隔）
    if i < 3:
        spacing = min_spacing  # 尾部 3 个之间保持 38mm
    else:
        spacing = min_spacing * (1 - progress) + max_spacing * progress
    
    # 杖身直径：底部粗 → 顶端细（外径）
    shaft_dia_external = shaft_base_diameter_external * (1 - t) + shaft_tip_diameter_external * t
    shaft_dia_internal = shaft_base_diameter_internal * (1 - t) + shaft_tip_diameter_internal * t
    
    # 瘤体长度：底部圆柱形 → 顶端椭球形
    bump_len = bump_dia * (1.2 + t * 0.4)
    
    # 位置计算
    if i == 0:
        pos = 50  # 第一个瘤体距手柄末端 50mm
    else:
        prev_spacing = node_data[i-1]["spacing"]
        pos = node_data[i-1]["pos"] + prev_spacing
    
    node_data.append({
        "pos": pos,
        "shaft_dia_external": shaft_dia_external,
        "shaft_dia_internal": shaft_dia_internal,
        "bump_dia": bump_dia,
        "bump_len": bump_len,
        "spacing": spacing,
        "is_tail": i < 3  # 前 3 个是尾部瘤体（圆柱形）
    })

# 绘制圆柱形手柄（中空，容纳 PCB）
def draw_cylindrical_handle(start_x, start_y, handle_length, handle_radius_ext, handle_radius_int, wand_angle_rad):
    wand_dir_x = math.cos(wand_angle_rad)
    wand_dir_y = math.sin(wand_angle_rad)
    perp_dir_x = -math.sin(wand_angle_rad)
    perp_dir_y = math.cos(wand_angle_rad)
    
    end_x = start_x + handle_length * wand_dir_x
    end_y = start_y + handle_length * wand_dir_y
    
    # 手柄主体（中空圆筒）
    num_segments = int(handle_length / 3)
    for i in range(num_segments):
        t = i / num_segments
        x = start_x + t * (end_x - start_x)
        y = start_y + t * (end_y - start_y)
        
        # 外壁
        for r in range(int(handle_radius_ext * 2.2), 0, -2):
            layer_t = r / (handle_radius_ext * 2.2)
            brightness = int(120 * layer_t + 50)
            
            cx = x - perp_dir_x * handle_radius_ext * 0.3
            cy = y - perp_dir_y * handle_radius_ext * 0.3
            
            draw.ellipse([
                int(cx - r), int(cy - r),
                int(cx + r), int(cy + r)
            ], fill=(int(brightness*0.5), int(brightness*0.35), int(brightness*0.25), 255))
        
        # 内壁（暗色表示中空）
        for r in range(int(handle_radius_int * 2.2), 0, -2):
            layer_t = r / (handle_radius_int * 2.2)
            brightness = int(60 * layer_t + 20)
            
            cx = x - perp_dir_x * handle_radius_int * 0.3
            cy = y - perp_dir_y * handle_radius_int * 0.3
            
            draw.ellipse([
                int(cx - r), int(cy - r),
                int(cx + r), int(cy + r)
            ], fill=(brightness, brightness, brightness, 255))
    
    # 手柄底部（带 USB-C 开孔）
    for r in range(int(handle_radius_ext * 2.3), 0, -2):
        t = r / (handle_radius_ext * 2.3)
        brightness = int(110 * t + 55)
        draw.ellipse([
            int(start_x - r), int(start_y - r),
            int(start_x + r), int(start_y + r)
        ], fill=(int(brightness*0.5), int(brightness*0.35), int(brightness*0.25), 255))
    
    # USB-C 开孔（底部中心）
    usb_hole_radius = 5 * scale
    draw.ellipse([
        int(start_x - usb_hole_radius), int(start_y - usb_hole_radius * 0.6),
        int(start_x + usb_hole_radius), int(start_y + usb_hole_radius * 0.6)
    ], fill=(30, 30, 30, 255))
    
    # 手柄底部装饰环
    ring_x = start_x + 25 * wand_dir_x
    ring_y = start_y + 25 * wand_dir_y
    ring_radius = handle_radius_ext * 1.05
    
    for r in range(int(ring_radius * 2), 0, -1):
        t = r / (ring_radius * 2)
        brightness = int(100 * t + 60)
        draw.ellipse([
            int(ring_x - r), int(ring_y - r),
            int(ring_x + r), int(ring_y + r)
        ], fill=(int(brightness*0.45), int(brightness*0.3), int(brightness*0.2), 255))

# 绘制杖身（中空，容纳电池）
def draw_wand_shaft(shaft_start_x, shaft_start_y, shaft_end_x, shaft_end_y, 
                    radius_ext_start, radius_int_start, radius_ext_end, radius_int_end, wand_angle_rad):
    """绘制中空杖身"""
    
    perp_dir_x = -math.sin(wand_angle_rad)
    perp_dir_y = math.cos(wand_angle_rad)
    
    segments = 485
    for i in range(segments):
        t = i / segments
        x = shaft_start_x + t * (shaft_end_x - shaft_start_x)
        y = shaft_start_y + t * (shaft_end_y - shaft_start_y)
        
        # 外径渐变
        diameter_ext = (shaft_base_diameter_external - t * (shaft_base_diameter_external - shaft_tip_diameter_external)) * scale
        radius_ext = diameter_ext / 2
        
        # 内径渐变
        diameter_int = (shaft_base_diameter_internal - t * (shaft_base_diameter_internal - shaft_tip_diameter_internal)) * scale
        radius_int = diameter_int / 2
        
        nx = -math.sin(wand_angle_rad)
        ny = -math.cos(wand_angle_rad)
        
        color_base = int(139 - t * 30)
        color_green = int(69 - t * 15)
        color_blue = int(19 - t * 4)
        
        # 绘制外壁
        draw.line([
            (x + nx * radius_ext, y + ny * radius_ext),
            (x - nx * radius_ext, y - ny * radius_ext)
        ], fill=(color_base, color_green, color_blue), width=int(diameter_ext))
        
        # 绘制内壁（暗色表示中空）
        if radius_int > 2:
            draw.line([
                (x + nx * radius_int, y + ny * radius_int),
                (x - nx * radius_int, y - ny * radius_int)
            ], fill=(50, 35, 25), width=int(diameter_int))

# 绘制瘤体（中空外壳）
def draw_bump(cx, cy, shaft_radius_ext, shaft_radius_int, bump_radius, bump_length, is_tail, wand_angle_rad):
    random.seed(42)
    
    wand_dir_x = math.cos(wand_angle_rad)
    wand_dir_y = math.sin(wand_angle_rad)
    perp_dir_x = -math.sin(wand_angle_rad)
    perp_dir_y = math.cos(wand_angle_rad)
    
    if is_tail:
        elongation = 1.5
    else:
        elongation = 1.3
    
    num_points = 40
    for layer in range(int(bump_radius * 2.2), 0, -2):
        t = layer / (bump_radius * 2.2)
        brightness = int(130 * t + 45)
        r_val = int(brightness * 0.5)
        g_val = int(brightness * 0.35)
        b_val = int(brightness * 0.25)
        
        points = []
        for i in range(num_points):
            angle_i = i * 2 * math.pi / num_points
            radius_var = layer * (1 + 0.08 * math.sin(angle_i * 3))
            
            major_r = radius_var * elongation
            minor_r = radius_var * 0.9
            
            local_x = math.cos(angle_i) * major_r
            local_y = math.sin(angle_i) * minor_r
            
            x = cx + local_x * wand_dir_x - local_y * perp_dir_x
            y = cy + local_x * wand_dir_y - local_y * perp_dir_y
            
            points.append((x, y))
        
        if len(points) >= 3:
            draw.polygon([(int(x), int(y)) for x, y in points], 
                        fill=(r_val, g_val, b_val, 255))
    
    # 表面纹理
    num_grooves = 6
    for i in range(num_grooves):
        groove_angle = i * 2 * math.pi / num_grooves
        for j in range(5):
            t = j / 5
            gx = cx + math.cos(groove_angle) * bump_radius * 0.4 * t
            gy = cy + math.sin(groove_angle) * bump_radius * 0.4 * t
            pit_size = int(bump_radius * 0.1 * (1 - t))
            
            draw.ellipse([
                int(gx - pit_size), int(gy - pit_size),
                int(gx + pit_size), int(gy + pit_size)
            ], fill=(45, 30, 20, 190))
    
    # 高光
    highlight_x = cx - bump_radius * 0.35 * perp_dir_x
    highlight_y = cy - bump_radius * 0.35 * perp_dir_y
    draw.ellipse([
        int(highlight_x - bump_radius*0.12), int(highlight_y - bump_radius*0.12),
        int(highlight_x + bump_radius*0.12), int(highlight_y + bump_radius*0.12)
    ], fill=(190, 160, 140, 170))

# 绘制杖尖 LED 组件
def draw_tip_with_led(tip_x, tip_y, tip_radius, wand_angle_rad):
    """绘制杖尖，带 LED 透光孔"""
    
    wand_dir_x = math.cos(wand_angle_rad)
    wand_dir_y = math.sin(wand_angle_rad)
    perp_dir_x = -math.sin(wand_angle_rad)
    perp_dir_y = math.cos(wand_angle_rad)
    
    cone_length = 60 * scale
    
    # 锥形杖尖
    for i in range(int(cone_length)):
        t = i / cone_length
        cone_x = tip_x - i * wand_dir_x
        cone_y = tip_y - i * wand_dir_y
        cone_radius = (shaft_tip_diameter_external/2 + t * 3) * scale
        
        nx = -math.sin(wand_angle_rad)
        ny = -math.cos(wand_angle_rad)
        
        draw.line([
            (cone_x + nx * cone_radius, cone_y + ny * cone_radius),
            (cone_x - nx * cone_radius, cone_y - ny * cone_radius)
        ], fill=(90, 55, 35), width=int(cone_radius * 2))
    
    # LED 透光孔（杖尖中心）
    led_hole_radius = 4 * scale
    draw.ellipse([
        int(tip_x - led_hole_radius), int(tip_y - led_hole_radius),
        int(tip_x + led_hole_radius), int(tip_y + led_hole_radius)
    ], fill=(20, 20, 20, 255))
    
    # LED 发光效果（蓝色）
    for i in range(18, 0, -1):
        alpha = i * 10
        glow_radius = i * 2.2
        draw.ellipse([
            int(tip_x - glow_radius), int(tip_y - glow_radius),
            int(tip_x + glow_radius), int(tip_y + glow_radius)
        ], fill=(100, 180, 255, alpha))

# 绘制按键开孔
def draw_button_holes(shaft_start_x, shaft_start_y, wand_angle_rad):
    """在手柄处绘制按键开孔"""
    
    perp_dir_x = -math.sin(wand_angle_rad)
    perp_dir_y = math.cos(wand_angle_rad)
    
    # 3 个按键开孔（手柄侧面）
    button_positions = [0.3, 0.5, 0.7]  # 手柄长度比例
    button_radius = 4 * scale
    
    for pos in button_positions:
        bx = shaft_start_x + pos * 80 * scale * math.cos(wand_angle_rad)
        by = shaft_start_y + pos * 80 * scale * math.sin(wand_angle_rad)
        
        # 按键孔（暗色表示凹陷）
        draw.ellipse([
            int(bx - button_radius), int(by - button_radius),
            int(bx + button_radius), int(by + button_radius)
        ], fill=(30, 30, 30, 255))

# 绘制魔杖主体
def draw_wand():
    handle_start_x = center_x - 400
    handle_start_y = center_y + 400
    end_x = handle_start_x + wand_total_length * scale * math.cos(rad)
    end_y = handle_start_y + wand_total_length * scale * math.sin(rad)
    
    # 1. 绘制圆柱形手柄（15mm 长，中空）
    handle_length = 15 * scale
    handle_radius_ext = handle_diameter_external * scale / 2
    handle_radius_int = handle_diameter_internal * scale / 2
    draw_cylindrical_handle(handle_start_x, handle_start_y, handle_length, 
                           handle_radius_ext, handle_radius_int, rad)
    
    handle_end_x = handle_start_x + handle_length * math.cos(rad)
    handle_end_y = handle_start_y + handle_length * math.sin(rad)
    
    # 2. 绘制中空杖身
    shaft_start_x = handle_end_x
    shaft_start_y = handle_end_y
    shaft_length = (wand_total_length - 15) * scale
    
    draw_wand_shaft(shaft_start_x, shaft_start_y, end_x, end_y,
                   shaft_base_diameter_external * scale / 2,
                   shaft_base_diameter_internal * scale / 2,
                   shaft_tip_diameter_external * scale / 2,
                   shaft_tip_diameter_internal * scale / 2,
                   rad)
    
    # 3. 绘制渐进瘤体
    for i, node in enumerate(node_data):
        t = node["pos"] / (wand_total_length - 15)
        node_x = shaft_start_x + t * (end_x - shaft_start_x)
        node_y = shaft_start_y + t * (end_y - shaft_start_y)
        
        shaft_radius_ext = node["shaft_dia_external"] * scale / 2
        shaft_radius_int = node["shaft_dia_internal"] * scale / 2
        bump_radius = node["bump_dia"] * scale / 2
        bump_length = node["bump_len"] * scale
        
        draw_bump(node_x, node_y, shaft_radius_ext, shaft_radius_int, 
                 bump_radius, bump_length, node["is_tail"], rad)
    
    # 4. 绘制按键开孔
    draw_button_holes(shaft_start_x, shaft_start_y, rad)
    
    # 5. 绘制杖尖 LED
    draw_tip_with_led(end_x, end_y, shaft_tip_diameter_external * scale / 2, rad)

# 添加标注
def draw_annotations():
    try:
        font_medium = ImageFont.truetype("msyh.ttc", 24)
    except:
        font_medium = ImageFont.load_default()
    
    annotations = [
        ("总长度：500mm（手柄 15mm + 杖身 485mm）", center_x - 280, center_y + 450),
        ("手柄外径：31mm（内径 28mm 容纳 PCB 25mm）", center_x - 280, center_y + 480),
        ("杖身外径：26-15mm（内径 23-12mm 容纳电池）", center_x - 280, center_y + 510),
        ("尾部瘤体：32mm + 间隔 38mm（圆柱形）", center_x - 280, center_y + 540),
        ("LED：杖尖 WS2812B（透光孔）", center_x - 280, center_y + 570),
        ("按键：手柄处 3 个开孔", center_x - 280, center_y + 600),
        ("USB-C：手柄底部开孔", center_x - 280, center_y + 630),
    ]
    
    for text, x, y in annotations:
        draw.text((x, y), text, fill='#2C1810', font=font_medium)

# 添加标题
def draw_title():
    try:
        font_large = ImageFont.truetype("msyh.ttc", 48)
        font_medium = ImageFont.truetype("msyh.ttc", 32)
    except:
        font_large = ImageFont.load_default()
        font_medium = ImageFont.load_default()
    
    title = "CyberWand v14.0 - PCB 兼容版"
    subtitle = "手柄 31mm 容 PCB | 杖身 26mm 容电池 | LED 透光"
    
    title_bg = Image.new('RGBA', (1800, 150), (0, 0, 0, 100))
    img.paste(title_bg, (center_x - 900, 80), title_bg)
    
    draw.text((center_x, 100), title, fill='#2C1810', font=font_large, anchor="mm")
    draw.text((center_x, 140), subtitle, fill='#6B3E1F', font=font_medium, anchor="mm")

# 执行绘制
print("Drawing PCB-compatible wand v14.0...")
draw_wand()
draw_annotations()
draw_title()

# 保存
output_path = r"D:\workspace\CyberWand_Insta360\enclosure\wand_v140_pcb_compatible.png"
img.save(output_path, 'PNG', quality=95)
print(f"Render complete! Saved to: {output_path}")
