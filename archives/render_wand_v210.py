"""
CyberWand v21.0 - 瘤体由密到疏版
基于 v20.0 最小化版，优化瘤体间隔分布

关键调整：
1. 瘤体间隔：从很密 (20mm) 到很疏 (50mm)
2. 瘤体数量：保持 6 个
3. 分布：底部密集 → 顶端稀疏（渐变明显）
"""
...
# 魔杖参数（v20.0 最小化）
wand_total_length = 350
handle_length = 12
handle_diameter = 22
shaft_base_diameter = 14
shaft_tip_diameter = 8

# 瘤体参数（6 个，由密到疏）
num_nodes = 6
max_bump_dia = 18.0  # 尾部最大
min_bump_dia = max_bump_dia * (1 - 3/7)  # 10.3mm 顶端最小

# 间隔参数：从很密 (20mm) 到很疏 (50mm)
min_spacing = 20  # 底部最密间隔
max_spacing = 50  # 顶端最疏间隔

# 生成由密到疏的瘤体分布
node_data = []
for i in range(num_nodes):
    t = i / (num_nodes - 1)
    progress = t * t  # 二次渐进，变化更明显
    ...
    # 间隔渐进：底部密 → 顶端疏（变化幅度加大）
    # 使用三次曲线，让间隔变化更明显
    spacing_progress = t * t * t  # 三次曲线，间隔变化更陡峭
    spacing = min_spacing * (1 - spacing_progress) + max_spacing * spacing_progress
...
def draw_cylindrical_handle(start_x, start_y, handle_length, handle_radius, wand_angle_rad):
    ...
    ring_x = start_x + 20 * wand_dir_x
    ring_y = start_y + 20 * wand_dir_y
...
# 绘制魔杖主体
def draw_wand():
    handle_start_x = center_x - 300
    handle_start_y = center_y + 300
    ...
    draw_cylindrical_handle(handle_start_x, handle_start_y, handle_length_scaled, handle_radius, rad)
    ...
    shaft_length = (wand_total_length - handle_length) * scale
    
    segments = int((wand_total_length - handle_length) * scale / 2)
    ...
    for i, node in enumerate(node_data):
        t = node["pos"] / (wand_total_length - handle_length)
    ...
    for i in range(int(cone_length)):
        ...
        cone_radius = (shaft_tip_diameter/2 + t * 2) * scale
...
def draw_annotations():
    ...
    # 显示实际间隔
    spacings = [node["spacing"] for node in node_data]
    
    annotations = [
        ("总长度：350mm（最小化）", center_x - 250, center_y + 420),
        ("手柄：Φ22mm × 12mm（容纳芯片）", center_x - 250, center_y + 450),
        ("杖身：Φ14-8mm（最小化）", center_x - 250, center_y + 480),
        ("瘤体分布：由密到疏（20mm→50mm）", center_x - 250, center_y + 510),
        (f"间隔：{spacings[0]:.0f}→{spacings[1]:.0f}→{spacings[2]:.0f}→{spacings[3]:.0f}→{spacings[4]:.0f}→{spacings[5]:.0f}mm", center_x - 250, center_y + 540),
        ("直径：18mm→10.3mm（渐进）", center_x - 250, center_y + 570),
    ]
...
def draw_title():
    ...
    title = "CyberWand v21.0 - 瘤体由密到疏版"
    subtitle = "间隔 20→50mm（由密到疏）| 6 个瘤体 | 总长 350mm"
...
# 执行绘制
print("Drawing wand with dense-to-sparse nodes v21.0...")
draw_wand()
draw_annotations()
draw_title()

# 保存
output_path = r"D:\workspace\CyberWand_Insta360\enclosure\wand_v210_dense_to_sparse.png"
